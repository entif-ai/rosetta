---
{
  'id': 'entif.essay.etr-2026-07.02',
  'slug': '02-language-that-makes-power-hard-to-see',
  'title': 'Language That Makes Power Hard to See',
  'description': 'Section 2 of Accelerating the Dystopia: Why Artificial Intelligence Cannot Save a System We Refuse to Fix. Language That Makes Power Hard to See.',
  'kind': 'essay',
  'status': 'published',
  ...
  'sourceRefs':
    [
      'https://drive.google.com/file/d/1AFxMc8oERtb0ONfF8ZZLbanf3If81R8l/view?usp=drivesdk',
    ],
  'series': { 'id': 'accelerating-the-dystopia', 'order': 2 },
}
---

There is a peculiar magic trick powerful institutions perform with words.
...
A package does not contain less product for the same sticker price. The brand executes a pack-size architecture adjustment.

A financial intervention is described in terms of liquidity support or market functioning. Those descriptions can be accurate while leaving its effects on private portfolios and asset prices unexplained. The shared words do not make the operations identical. Their balance-sheet mechanics matter.

None of those phrases has to be false.
...
So I want a hard distinction throughout this essay:

**Disclosure is not transparency.**

Disclosure is the presence of information in a formally available location.

Transparency is the practical ability of a relevant human being to perceive, understand, compare, and act on it before the decision becomes costly to reverse.

Those are different engineering objectives.

The same distinction applies to expertise. Specialized fields require specialized vocabulary. I do not expect a cardiologist to replace "ventricular fibrillation" with "the heart is doing a weird little dance" for the sake of accessibility. Precision matters. Technical distinctions matter. The problem begins when the specialist controls both the underlying complexity and the way outsiders are permitted to encounter it, then treats the outsider's inability to contest the explanation as evidence that the explanation has been accepted.

The Federal Reserve preserved a useful historical account. In a November 2009 oral-history session, Alan Greenspan recalled a tradition of deliberately maintaining uncertainty about policy to support two-sided markets. He also described that rationale weakening as markets grew. The published transcript notes that he did not review it. This is a retrospective account of a communication strategy, not proof that all obscure monetary language is deceptive. [S103]

And this is where vocabulary becomes power rather than mere style.

Sociologists Wendy Espeland and Mitchell Stevens describe **commensuration** as the process of transforming different qualities into a common metric. [S104] That sounds abstract until you notice how much of modern institutional life depends on it. Credit scores. University rankings. Employee performance bands. ESG scores. Vendor quadrants. Risk ratings. Customer lifetime value. Engagement. Net promoter scores. Productivity metrics. Sovereign credit ratings. "Talent density." Every one of these instruments compresses messy reality into a form that can be compared.

Compression is useful. It is also political.
...
> We recommend eliminating 10,000 jobs and automating as much of the remaining work as possible.

and saying:

> We recommend rightsizing the workforce while accelerating digitally enabled productivity capture across the target operating model.

The second sentence does not necessarily contain a factual lie. It changes the object of attention. People disappear into "workforce." Firing becomes "rightsizing." The human consequences become transition costs. The moral decision is converted into an administrative transformation.
...
Eventually people outside the institution experience the consequence in plain English while being told the cause in a language they do not speak.

Their bill went up.

Their package got smaller.

Their job disappeared.

Their rent increased.

Their mortgage became unaffordable.

Their neighborhood was bought by investors.

Their pension depends on the market going up.

Their government spends a trillion dollars a year on interest.
...
Gabaix and Laibson's model of shrouded attributes is important here because it does not assume competition automatically makes every hidden term clear. Under their model's conditions, firms can profit from consumers who overlook add-on costs, and revealing those costs need not be the winning strategy for a rival. That is a model of a mechanism, not proof that every complex price has the same cause. [S101]

The Federal Trade Commission's dark-pattern report documents a related practical problem: interfaces that hide material information, make cancellation difficult, disguise advertising, or steer privacy choices. Its categories show how a formally available choice can be shaped by presentation and friction. [S102]

What I draw from these sources is a distinction between information scarcity and usable-understanding scarcity. More words do not necessarily solve the latter. An AI system that generates an excellent explanation could help a customer. An AI system that generates an excellent explanation of why the customer's objection does not qualify could help an institution maintain the same barrier more efficiently.

The difference lies partly in who gets to define the question. “Explain the charges” is not the same task as “help this person establish whether the charges match the offer.” One produces a description. The other compares a promise, a bill, and a remedy. A system can perform the first beautifully while never being asked to perform the second.

## The cost of making a claim

The administrative-burden literature distinguishes learning costs, compliance costs, and psychological costs. A person has to find out how a process works, do what it requires, and endure the experience of navigating it. Moynihan, Herd, and Harvey developed that framework in research on citizen-state interactions, including a historical Medicaid case. The framework does not establish that every difficult process was designed to discourage participation. It identifies costs that a simple count of completed transactions can miss. [S205]

The distinction travels usefully into a hypothetical customer dispute. Learning the procedure is different from finding a document. Finding the document is different from having time to upload it. Completing the form is different from tolerating another rejection written in language that gives no useful next step. We should not treat these stages as one undifferentiated failure of effort by the person trying to proceed.

A system can therefore filter people without ever saying that they are excluded. It can require a sequence of small tasks whose combined cost exceeds what some people can bear. A person with spare time, stable internet access, organized records, and confidence in dealing with institutions may complete the sequence. Another may abandon it. The resulting data will show different rates of completion. It will not, by itself, explain why.

This is where automation creates two distinct possibilities. It can reduce the burden by carrying information across steps, explaining requirements, and helping repair a mistake. Or it can increase the rate at which institutions generate demands for information. A cheap, personalized request is still a request someone must answer. Removing work from one side of an exchange does not prove that total work has fallen.
...
Technical language is not the enemy. I need it when distinctions matter. A Treasury buyback is not quantitative easing. Revenue is not profit. Statistical association is not causation. An observed model behavior is not an authenticated report of the model's internal state. Removing those distinctions would make this essay easier to read in the worst possible way: by making it less true.
...
There is a second danger: fluent explanation can create confidence without understanding. An answer may feel clear because every sentence is smooth, even when the underlying relationship remains unclear. A person can understand every word in “your request does not meet the relevant criteria” and still have no idea which criterion failed or what evidence could change the decision.
...
I am not proposing that every answer become a legal manual. I am drawing a line between language that describes a wall and language that identifies a door. A human-facing system needs to know which of those tasks it has actually completed.
...
The same problem appears when a company reports average customer satisfaction after excluding customers who never reached the survey, or reports average response time without measuring whether the answer helped. These are illustrative measurement designs. Their lesson is logical: a metric can be accurate about the selected records and incomplete about the experience it appears to represent.
...
This is an analytical possibility, not a claim that all models necessarily enter that loop. It follows from the sequence: define a target, reward behavior that matches it, record the rewarded behavior, and train on the record. The loop contains learning, but the learning is not free from the original choice.
