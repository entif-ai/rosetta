...
test('homepage renders the Entif identity and navigable research', async ({ page }) => {
  await page.goto('./');
...
test('topic filter works without navigating away', async ({ page }) => {
  ...
  await expect(page.getByRole('article').filter({ hasText: 'Agentic memory' })).toBeVisible();
});
...
test('mobile viewport does not produce horizontal page overflow', async ({ page }) => {
...
test('homepage and article have no serious or critical axe findings', async ({ page }) => {
  await page.goto('./');
